...
class Player extends React.Component {
    ...
    componentDidMount() {
        this.getPlayer().then((res) => {
            ...
            return this.getIndex(res);
        })
        .then(({player, index}) => {
            this.setState({splits: player.stats[index]});
        }) 
        .then(() => {
            let total = this.aggregateData(this.state.splits);
            this.setState({aggTableRow: this.makeTableRow(total)});
        })
        .catch((err) => {
            console.log(err);
        })
